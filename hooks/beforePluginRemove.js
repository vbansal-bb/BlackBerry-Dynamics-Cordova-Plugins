...
module.exports = (context) => {
  if (context.opts.plugins[0] !== "cordova-plugin-bbd-media-capture") {
    return;
  }
  ...
  const cleanupCordovaPlugins = (cordovaPluginsPath) => {
    try {
      if (fs.existsSync(cordovaPluginsPath)) {
        ...
        const startString = "module.exports = ",
          ...
          updatedPluginList = pluginsList.filter((plugin) => {
            if (deleteFilePluginRef)
              return (
                plugin.pluginId !== "cordova-plugin-bbd-media-capture" &&
                plugin.pluginId !== "cordova-plugin-bbd-file"
              );
            else return plugin.pluginId !== "cordova-plugin-bbd-media-capture";
          }),
        ...
        if ("cordova-plugin-bbd-media-capture" in metaDataObj) {
          delete metaDataObj["cordova-plugin-bbd-media-capture"];
      ...
      console.log(
        "cordova-plugin-bbd-media-capture plugin's cleanup hook error = ",
        error.message
      );
    }
  };
  console.log(
    "cordova-plugin-bbd-media-capture plugin cleanup hook execution started"
  );
  ...
  console.log(
    "cordova-plugin-bbd-media-capture plugin cleanup hook execution finished"
  );
};
