...
(function () {
  ...
  for (const [script, integrationHookFileName] of Object.entries(hooks)) {
    ...
    if (
      ...
      packageJson.scripts[script].indexOf(hook) > 0
    ) {
      break;
    }
    ...
    if (
      packageJson.scripts[script] &&
      !packageJson.scripts[script].includes(hook)
    ) {
  ...
  if (!isWindows) {
    ...
    const fileContext = fs.readFileSync(cordovaPluginsPodsSpecPath, {
      encoding: "utf8",
    });
    ...
    const { dynamicsPodSpec } = JSON.parse(
      fs.readFileSync(path.join(bbdBasePath, "package.json"), "utf-8")
    );
    ...
    replaceAndSave(
      capacitorPodFile,
      [
        [/platform :ios, \'([\d\.\d]+)\'/, "platform :ios, '16.0'"],
        [
    ...
    const capacitorPodSpecFile = path.join(
      projectRoot,
      "node_modules",
      "@capacitor",
      "ios",
