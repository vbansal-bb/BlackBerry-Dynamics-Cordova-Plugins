...
module.exports = (context) => {
  if (context.opts.plugins[0] !== "cordova-plugin-bbd-file-transfer") {
    return;
  }
  ...
  const cleanupCordovaPlugins = (cordovaPluginsPath) => {
    try {
      if (fs.existsSync(cordovaPluginsPath)) {
        ...
        const startString = "module.exports = ",
          ...
          updatedPluginList = pluginsList.filter((plugin) => {
            if (deleteFilePluginRef)
              return (
                plugin.pluginId !== "cordova-plugin-bbd-file-transfer" &&
                plugin.pluginId !== "cordova-plugin-bbd-file"
              );
            else return plugin.pluginId !== "cordova-plugin-bbd-file-transfer";
          }),
        ...
        if ("cordova-plugin-bbd-file-transfer" in metaDataObj) {
          delete metaDataObj["cordova-plugin-bbd-file-transfer"];
      ...
      console.log(
        "cordova-plugin-bbd-file-transfer plugin's cleanup hook error = ",
        error.message
      );
    }
  };
  console.log(
    "cordova-plugin-bbd-file-transfer plugin cleanup hook execution started"
  );
  ...
  console.log(
    "cordova-plugin-bbd-file-transfer plugin cleanup hook execution finished"
  );
};
