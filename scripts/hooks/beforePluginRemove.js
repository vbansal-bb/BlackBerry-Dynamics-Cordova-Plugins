...
module.exports = (context) => {
  if (context.opts.plugins[0] !== "cordova-plugin-bbd-file") {
    return;
  }
  ...
  for (const plugin of otherPluginsUsingBBDFile) {
    if (context.opts.cordova.plugins.includes(plugin)) {
      console.log(
        "File module cleanup hook can not run as other plugin using it and hence can't be uninstalled"
      );
      return;
    }
  }
    ...
    try {
      if (fs.existsSync(cordovaPluginsPath)) {
        ...
        const startString = "module.exports = ",
          ...
          updatedPluginList = pluginsList.filter(
            (plugin) => plugin.pluginId !== "cordova-plugin-bbd-file"
          ),
        ...
        if ("cordova-plugin-bbd-file" in metaDataObj) {
          delete metaDataObj["cordova-plugin-bbd-file"];
      ...
      console.log(
        "cordova-plugin-bbd-file plugin's cleanup hook error = ",
        error.message
      );
    }
  };
  console.log("cordova-plugin-bbd-file plugin cleanup hook execution started");
  ...
  console.log("cordova-plugin-bbd-file plugin cleanup hook execution finished");
};
