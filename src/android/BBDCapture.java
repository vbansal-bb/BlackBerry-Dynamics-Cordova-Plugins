/*
       ...
       under the License.
*/
package com.blackberry.bbd.cordova.plugins.mediacapture;
...
import com.blackberry.bbd.cordova.plugins.mediacapture.PendingRequests.Request;
...
public class BBDCapture extends CordovaPlugin {
    ...
    private static final String GD_MEDIA_FOLDER_PATH = "Inbox/media";
    ...
    private void captureAudio(Request req) {
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.Q) {
            final String errorMsg = "Secure audio capture is not supported on Android API Level " + android.os.Build.VERSION.SDK_INT;
            Log.e(LOG_TAG, errorMsg);
            pendingRequests.resolveWithFailure(req, createErrorObject(CAPTURE_NOT_SUPPORTED, errorMsg));
            return;
        }
        boolean needWriteExternalStoragePermission =
                !PermissionHelper.hasPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE);

        boolean needMicrophonePermission = !PermissionHelper.hasPermission(this, Manifest.permission.RECORD_AUDIO);

        if (!PermissionHelper.hasPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
            PermissionHelper.requestPermission(this, req.requestCode, Manifest.permission.READ_EXTERNAL_STORAGE);
        } else if (needWriteExternalStoragePermission || needMicrophonePermission) {
            if (needWriteExternalStoragePermission && needMicrophonePermission) {
                PermissionHelper.requestPermissions(this, req.requestCode, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE,
    ...
    private void captureVideo(Request req) {
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.Q) {
            final String errorMsg = "Secure video capture is not supported on Android API Level " + android.os.Build.VERSION.SDK_INT;
            Log.e(LOG_TAG, errorMsg);
            pendingRequests.resolveWithFailure(req, createErrorObject(CAPTURE_NOT_SUPPORTED, errorMsg));
            return;
        }
    ...
    private File moveMediaFileToSecureFilesystem(File file)
            throws IOException {
        InputStream inputStream = null;
        OutputStream outputStream = null;
        File secureFile = null;
        try {
            File gdMediaFolder = gdFileSystemDelegate.createFile(GD_MEDIA_FOLDER_PATH);
            if (!gdMediaFolder.exists()) {gdMediaFolder.mkdir();}

            String gdMediaPathFolder = null;
            String mimeType = getMimeTypeHelper(file);
            if (mimeType.contains("image")) {
                File imagesDir = gdFileSystemDelegate.createFile(GD_MEDIA_FOLDER_PATH + File.separator + "images");
                if (!imagesDir.exists()) {imagesDir.mkdir();}
                gdMediaPathFolder = GD_MEDIA_FOLDER_PATH + File.separator + "images" + File.separator + file.getName().replace(" ", "_");
            } else if (mimeType.contains("audio")) {
                File audioDir = gdFileSystemDelegate.createFile(GD_MEDIA_FOLDER_PATH + File.separator + "audio");
                if (!audioDir.exists()) {audioDir.mkdir();}
                gdMediaPathFolder = GD_MEDIA_FOLDER_PATH + File.separator + "audio" + File.separator + file.getName().replace(" ", "_");
            } else if (mimeType.contains("video")) {
                File videoDir = gdFileSystemDelegate.createFile(GD_MEDIA_FOLDER_PATH + File.separator + "video");
                if (!videoDir.exists()) {videoDir.mkdir();}
                gdMediaPathFolder = GD_MEDIA_FOLDER_PATH + File.separator + "video" + File.separator + file.getName().replace(" ", "_");
            }

            secureFile = gdFileSystemDelegate.createFile(gdMediaPathFolder);
            if (secureFile.createNewFile()) {
                outputStream = gdFileSystemDelegate.openFileOutput(gdMediaPathFolder, GDFileSystem.MODE_PRIVATE);
    ...
    private void deleteMediaFile(File file) {
        final String mimeType = getMimeTypeHelper(file);

        if (mimeType.contains("image")) {
            deleteImageFromGalary(file);
        } else if (mimeType.contains("audio")) {
            deleteAudioFromGalary(file);
        } else if (mimeType.contains("video")) {
            deleteVideoFromGalary(file);
        }
    }

    private void deleteImageFromGalary(File file) {
    ...
    private void deleteAudioFromGalary(File file) {
        // Set up the projection (we only need the ID)
    ...
    private void deleteVideoFromGalary(File file) {
        // Set up the projection (we only need the ID)
    ...
    private JSONObject createMediaFile(Uri data) {
        ...
        File secureFile;
        try {
            secureFile = moveMediaFileToSecureFilesystem(fp);
            ...
            FileUtils filePlugin = (FileUtils) pm.getPlugin("BBDFile");
            LocalFilesystemURL url = filePlugin.filesystemURLforLocalPath(secureFile.getAbsolutePath());

            try {
                // File properties
                obj.put("name", secureFile.getName());
                obj.put("fullPath", Uri.fromFile(secureFile));
                if (url != null) {
                    obj.put("localURL", url.toString());
                }

                obj.put("lastModifiedDate", secureFile.lastModified());
                obj.put("type", getMimeTypeHelper(secureFile));
                obj.put("size", secureFile.length());
    ...
    private String getMimeTypeHelper(File file) {
        ...
        if (file.getAbsoluteFile().toString().endsWith(".3gp") || file.getAbsoluteFile().toString().endsWith(".3gpp")) {
            if (Uri.fromFile(file).toString().contains("/audio/")) {
                return AUDIO_3GPP;
            } else {
                return VIDEO_3GPP;
            }
        } else {
            return FileHelper.getMimeType(Uri.fromFile(file), cordova);
        }
    }
